...
// Import XLS ta table
//----------------------------------------------------------------------
function import_xls($target_file,$try){
...
//----------------------------------------------------------------------
// Import XLSX ta table
//----------------------------------------------------------------------
function import_xlsxold($target_file,$try){
...
function importFromTable($table,$tableName,$try,$searchKeys,$primary,$generalFilter=[],$majorKey=""){
    ...
    echo "<h2>LOOP import from table</h2>";
    for ($i=1;$i<sizeof($table)-1;$i++){
        ...
        for ($k=0;$k<sizeof($line);$k++){
            ...
            if(($line[$k]!="")&&(in_array($headers[$k],$generalFilter))){
                //echo "accepted<br>";
                $input[$headers[$k]]=addslashes($line[$k]);
            }
        }
        //echo "<br>input<br>";
        //dispArray($input);
        ...
        if (!isset($input[$majorKey])){$input[$majorKey]="";} //make sure $majorkey exist for following test
        ...
        //echo "<br>major key=".$input[$majorKey]."<br>";
        ...
        if ($input[$majorKey]!=""){
            if (sizeof($resultTable)==1){
                ...
                if (sizeof($input)>0){
                    echo "<br>La ligne avec les informations va être créée dans '$tableName'<br>";
