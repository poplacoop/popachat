...
if ($order['departement']!=""){
    $where=" and departement=".$order['departement'];
}
if ($order['famille']!=""){
...
foreach ($filterList as $item){
    ...
    //displayinhtml($itemTable);
    //echo "item=".$item." ";
    $str.= listeSelect($filter,$item,$item,$order[$item],1); // the 1 if for order
}
...
//if ($order['graph']!=""){$special=array_merge($special,['graph']);}
        $str.="<table>
        <tr><th>code barre</th><th>référence Fournisseur</th><th>libellé</th></tr>
        <tr id='search' >
            ...
            <td><a id='exportXls' href='' onclick='exportXls();' target='_blank'>ExportExcel</a></td>
        </tr>
        </table>"; 
        //017-exportXls
