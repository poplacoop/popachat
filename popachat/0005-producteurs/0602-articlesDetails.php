...
if (isset($_REQUEST['activate'])){
    if ($order['validated']=="on"){$validated=1;}else{$validated=2;}
    $query="UPDATE prod_articles SET validated=$validated where ean=".$order['ean'];
    //echo $query;
    simple_query($query);
...
if (isset($_REQUEST['dateStock'])){
    if ($_REQUEST['dateStock']!=""){
        $query="INSERT into prod_stock_mvt (thedate,bl,ean,ajout,raison,source,user,type) VALUES 
        ('".$_REQUEST['dateStock']."','POPinventaire','".$order['ean']."','".$_REQUEST['newStock']."','INVENTAIRE','SITE','".$_SESSION['userInfo']['userId']."','I')";
        echo $query;
        simple_query($query);
        //header("Location:0602-articlesDetails.php?ean=$ean&myrange=".$order['myRange']);
    }
}
...
// find next article
//
//echo "departement".$_SESSION['departement'];
//echo "famille".$_SESSION['famille'];

$where="";
if (isset($_SESSION['fournisseur'])){if ($_SESSION['fournisseur']>0){$where.=" and fournisseur=".$_SESSION['fournisseur'];}}
if (isset($_SESSION['famille'])){if ($_SESSION['famille']>0){$where.=" and famille=".$_SESSION['famille'];}}
if (isset($_SESSION['departement'])){if ($_SESSION['departement']>0){$where.=" and departement=".$_SESSION['departement'];}}

if ($_SESSION['tridisp']==1){$queryTri=" order by tri,designation";}else{$queryTri=" order by designation";}

$query="SELECT * FROM prod_articles where validated<2 $where $queryTri";
//echo $query;
...
$nextEan=$tableDico[$nb]['ean'];
//echo "nb=$nb";
if ($nb>1){
    $nb=$nb-2;
}
else{
    $nb=sizeof($tableDico)-1;
}
//echo "nb=".$nb;
$prevEan=$tableDico[$nb]['ean'];
...
<input name='departement' type='hidden' value=".$_SESSION['departement']."></input>
<input name='famille'  type='hidden' value=".$_SESSION['famille']."></input>
<input name='fournisseur' type='hidden' value=".$_SESSION['fournisseur']."></input>
...
//if (!isset($stockDico)){
    $stockDico=getStockValue($order,$order['ean']);
//}
//dispArray($data['sales']);
$str=graphStockNew($stockDico,$designation);
echo $str;
...
if ($_SESSION['userInfo']['admin']){
    echo "Ajustement des stocks<input type='date' name='dateStock'></input>
    ...
    echo "<input type='hidden' name='myRange' value='".$order['myRange']."'></input>";
    echo "</table>";
...
if ($_SESSION['userInfo']['admin']){
    $checked="";
    $val=1;
    if ($tableArticle[1]['validated']<2){$checked="checked";$val=0;}
    echo "<div class='theRange'><button name='activate' value='$val'>Activation</button>\n";
    
    echo "
        <div class='form-check form-switch'>
              <input class='form-check-input' type='checkbox' id='flexSwitchCheckDefault' name='validated' $checked >
            </div>";
    echo "</div>";  
}
echo "</form>";
