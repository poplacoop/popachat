...
function getStockValue($order){
    ...
    if (sizeof($table)>1){
        ...
        $initStock=0; 
        $noInitialStock=1;
        $initDate=date("Y-m-d",time()-$order['myRange']*7*24*3600);
    }
    ...
    for ($day=1;$day<$maxDay;$day++){
        ...
        if (isset($data['inventaire'][$curDate])){
            $data['diffStock'][$curDate]+=($data['inventaire'][$curDate])-$data['stock'][$prevDate];
        ...
        if (isset($data['sales'][$curDate])){
            $data['stock'][$curDate]+=$data['stock'][$prevDate]-$data['sales'][$curDate];
        ...
        if (isset($data['inventairePop'][$curDate])){
            ...
            $data['stock'][$curDate]+=$data['diffStock'][$curDate];
            //echo "lastInventaire ".$lastInventaireDate.">".$curDate."<br>";
            if ($lastInventaireDate<$curDate){$lastInventaireDate=$curDate;}
        }
        
        //echo "<br>Stock=".$data['stock'][$curDate];
        //echo " sales ".$sales;
        //echo " real stock=".$data['realstock'][$curDate]."<br>";
        
        /*if (isset($data['realstock'][$curDate])){
            echo "On $curDate delta is ".(-$data['stock'][$curDate]+$data['realstock'][$curDate])."<br>";
            if (isset($data['realstock'][$curDate])){$realstock=$data['realstock'][$curDate];}
            //if (isset($data['inventaire'][$curDate])){$realstock=$data['inventaire'][$curDate];}
            
            $delta=-$realstock+$data['stock'][$curDate];
            //$delta=0;
            $data['stock'][$curDate]=$realstock;
            
            // correct for first data
            if ($noInitialStock*0==0){
                // back propagate
                $backCurDate=$curDate;
                $backPrevDate=date("Y-m-d",strtotime($backCurDate)-3600*24);
                
                while ($backCurDate>=$initDate){
                    //echo "correction $backCurDate ";
                    $data['stock'][$backCurDate]-=$delta;
                    //echo $data['stock'][$backCurDate]."<br>";
                    $backCurDate=$backPrevDate;
                    $backPrevDate=date("Y-m-d",strtotime($backCurDate)-3600*24);
                    
                }
                // back propagate
                $backCurDate=$curDate;
                $nextDate=date("Y-m-d",strtotime($backCurDate)+3600*24);
                //$data['stock'][$nextDate]=-$delta;
                $data['stock'][$curDate]=$realstock;
                echo "current stock".$data['stock'][$curDate];
                //while ($backNextDate<date("Y-m-d")and(!isset($data['bl'][$backNextDate]))){
                //    $data['stock'][$backNextDate]-=$delta;
                //    $backCurDate=$backNextDate;
                //    $backNextDate=date("Y-m-d",strtotime($backCurDate)+3600*24);
                //    echo "correction $backCurDate<br>";
                //}
                $noInitialStock=0;
            }
